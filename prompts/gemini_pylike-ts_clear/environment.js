...
exports.Environment = exports.EvaluationError = void 0;
...
class Environment {
    ...
    convert(magnitude, source, target) {
        const sourceStr = (0, index_1.serializeUnit)(source);
        const targetStr = (0, index_1.serializeUnit)(target);
        if (sourceStr === targetStr) {
            return new index_1.Value(magnitude, target);
        }
        let currentLevel = [{
                u: source,
                uStr: sourceStr,
        ...
        while (currentLevel.length > 0) {
            ...
            for (const state of currentLevel) {
                for (const rel of this.relationships.values()) {
                    ...
                    if (targetPower !== 0) {
                        ...
                        const poweredBasis = (0, index_1.powerUnit)(rel.basis, targetPower);
                        const nextU = (0, index_1.multiplyUnit)(newU, poweredBasis);
                        const nextUStr = (0, index_1.serializeUnit)(nextU);
                    ...
                    if (hasBasisMatch && allSameSign && sign !== 0 && minAbsE !== Infinity) {
                        ...
                        const basisE = (0, index_1.powerUnit)(rel.basis, e);
                        let nextU = (0, index_1.divideUnit)(state.u, basisE);
                        nextU[rel.target] = (nextU[rel.target] || 0) + e;
                        nextU = (0, index_1.normalizeUnit)(nextU);
                        const nextUStr = (0, index_1.serializeUnit)(nextU);
                        if (!visited.has(nextUStr)) {
            ...
            if (nextLevelMap.has(targetStr)) {
                return new index_1.Value(nextLevelMap.get(targetStr).state.mag, target);
            }
            currentLevel = [];
    ...
    addTransition(map, state, relTarget, isForward) {
